        ...
        if(global.__storage_external != "")
        {
            ...
            if(saf_file_exists("/", "test.ogg")) {
                if(test_audio == undefined) {
                    ...
                    if(test_audio != undefined) {
                        test_audio_id = audio_play_sound(test_audio, 0, false, 1);
                    }
                }
            }
            if(saf_file_exists("/", "test.txt")) {
                if(test_text == -1) {
                    ...
                    if(test_text != -1) {
                        while(saf_file_text_eof(test_text) != 0) {
            ...
            if(point_in_rectangle(mouse_x, mouse_y, room_width/8*2, room_height/32*25, room_width/8*6, room_height/32*27))
            {
                intent_saf_request(SAF_REQUEST_SEARCH_FILE);
            }
        }
    }
}